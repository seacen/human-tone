...
既想装上 human-tone skill、又想让规则常驻进你的 agent（写东西就自动去味），**一条命令从网上直接装，不用 clone**：
...
（等价写法：`npx github:seacen/human-tone --write`。）
...
--lang zh|en|both             # 默认 both
--agents claude,codex         # 只装指定 agent
--uninstall --write           # 一键撤销
```

这些参数接在上面命令后面即可（curl 版接在 `- ` 之后，如 `… | node --input-type=module - --write --lang zh`）。需要 Node（`npx skills` 本就依赖它；一份 Node 脚本 Mac / Windows 都能跑）。
